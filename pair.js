...
break;
  ...
  break;
    ...
    if (!query) {
        return await socket.sendMessage(sender, {
            text: '[❗] TikTok එකේ මොකක්ද බලන්න හදන්නේ උබ 😒! 🔍'
        }, { quoted: msg });
    }
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('API එකේ කේස් එකක් බන් news ගන්න බැරි උනා බන් දුකයි නේද 😩');
        }
        ...
        try {
            ...
            console.warn(`Thumbnail scrape කරන්න බැරි වුණා අඩන්න එපා හරිද 🥺 from ${link}: ${err.message}`);
        }
        ...
        await socket.sendMessage(sender, {
            text: '⚠️ නිව්ස් ගන්න බැරි වුණා 𝐁𝐌! 😩 යමක් වැරදුණා වගේ දුක් වෙන්න එපා මම අඩන ගමන් ඉන්නේ.'
        });
    }
               case 'nasa':
    try {
        ...
        await socket.sendMessage(sender, {
            text: '⚠️ඇයි උබ නාසා එකට යන්නද තාම කේල්ලෙක් නැනේද 😂'
        });
    }
    break;
                case 'news':
                    try {
                        ...
                        console.error(`Error in 'news' case: ${error.message}`);
                        await socket.sendMessage(sender, {
                            text: '⚠️ news බලන වෙලාවෙ tv එක බලපම්කො කේල්ලෙක් නැති එකා 😂'
                        });
                    }
                    break;
                case 'cricket':
                    try {
                        ...
                        await socket.sendMessage(sender, {
                            text: '⚠️ Cricket ගහන්නත් බැ මහලොකුවට බලනවා අනේ ඉතිම් කාට කියන්නද මේවා 😂.'
                        });
                    }
                    break;
                case 'song': {
                ...
                case 'winfo':
                    ...
                    if (inputNumber.length < 10) {
                        await socket.sendMessage(sender, {
                            ...
                            caption: formatMessage(
                                '❌ ERROR',
                                'Invalid phone number!(පකයට බුලත් දෙන්න බැ +94 ගහපම් 🥲)(e.g., +94741856766)',
                                '> 𝐏𝐎𝐖𝐄𝐑𝐃 𝘽𝙔 𝐀𝐒𝐇𝐈𝐘𝐀-𝐌𝐃 🥷🇱🇰'
                            )
                        });
                        break;
...
//පකයා මාමා 
  ...
  if (!q || q.trim() === '') {
    return await socket.sendMessage(sender, {
      text: "O කියන්න සුදු මම ASHIYA-AI 🥷"
    }, { quoted: msg });
  }

  const prompt = `ඔබගේ නම ASHIYA 𝐀𝐈.  
...
User Message: ${q}
  ...
  try {
    ...
    await socket.sendMessage(sender, {
      text: "❌ අයියෝ හිකිලා වගේ ‌‌‌අඩපන් දැන් 😢"
    }, { quoted: msg });
  }
...
router.get('/ping', (req, res) => {
    res.status(200).send({
        status: 'active',
        message: '🥷 𝐀𝐒𝐇𝐈𝐘𝐀-𝐌𝐃  𝐅𝚁𝙴𝙴 𝐁𝙾𝚃 වැඩ හුත්තො',
